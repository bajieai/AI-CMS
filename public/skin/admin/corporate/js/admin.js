...
(function () {
    ...
    window.showToast = function (msg, type, duration) {
        ...
        var html = '<div class="toast ui-toast align-items-center border-0 position-fixed top-0 end-0 m-3" role="alert" id="' + id + '" style="' + style + '">' +
            '<div class="d-flex align-items-center"><i class="bi ' + icon + ' fs-5 me-2 ms-2"></i>' +
            '<div class="toast-body">' + msg + '</div>' +
            (duration !== 0 ? '<button type="button" class="btn-close btn-close-white ms-2 me-1" data-bs-dismiss="toast"></button>' : '') +
            '</div></div>';
        $('body').append(html);
        var $toast = $('#' + id);
