...
class TemplateStoreController extends AdminBaseController
{
    ...
    {
        $service = new TemplatePromotionService();
        $params = $this->request->get();
        $data = $service->getList($params);
        ...
        return $this->view('/template_store/promotions');
    }

    /**
     * V2.9.31 T3-1: 新建/编辑推广活动页
     */
    public function promotionAdd()
    {
        $service = new TemplateStoreService();
        $categories = $service->getCategories();
        $templates = TemplateStore::where('status', TemplateStore::STATUS_ONLINE)->column('name', 'id');
    ...
    public function promotionEdit(int $id)
    {
        $info = \app\common\model\TemplatePromotion::find($id);
        if (empty($info)) {
    ...
    public function promotionSave()
    {
        ...
        try {
            $service->createPromo($data);
            return $this->success('保存成功', ['redirect' => '/admin/template_store/promotions']);
        } catch (\Throwable $e) {
            return $this->error('保存失败：' . $e->getMessage());
        }
    }
    ...
    /**
     * V2.9.31 T3-1: 切换推广活动状态
     */
    public function promotionToggle(int $id): \think\Response
    {
        $promo = \app\common\model\TemplatePromotion::find($id);
        if (empty($promo)) {
            return $this->error('活动不存在');
        }
     ...
     * V2.9.31 T3-1: 删除推广活动
     */
    public function promotionDelete(int $id): \think\Response
    {
        $promo = \app\common\model\TemplatePromotion::find($id);
        if (empty($promo)) {
            return $this->error('活动不存在');
        ...
        $promo->delete();
        (new TemplatePromotionService())->clearCache();
        return $this->success('删除成功');
    }
    ...
    {
        $service = new TemplateInstallLogService();
        $logs = $service->getRecentLogs(50);
        $trend = $service->getTrend(7);
        ...
        return $this->view('/template_store/install_logs');
    }
    ...
    /**
     * V2.9.31 CUS-1: 布局预设列表
     */
    public function layoutPresets()
    {
        $service = new \app\common\service\template\TemplateLayoutPresetService();
        $presets = $service->getAll();

        $this->assign('presets', $presets);
        return $this->view('/template_store/layout_presets');
    }

    /**
     * V2.9.31 CUS-1: 应用布局预设
     */
    public function applyLayoutPreset(): \think\Response
    {
        $data = $this->request->post();
        $memberId = (int) session('user_id');
        ...
        $service = new \app\common\service\template\TemplateLayoutPresetService();
        $result = $service->apply($memberId, $themeSlug, $presetKey);

        if ($result) {
            return $this->success('布局预设已应用');
        }
        return $this->error('应用失败');
    }

    /**
     * V2.9.31 CUS-2: 配色方案列表
     */
    public function colorSchemes()
    {
        $service = new \app\common\service\template\TemplateColorSchemeService();
        $schemes = $service->getAll();

        $this->assign('schemes', $schemes);
        return $this->view('/template_store/color_schemes');
    }

    /**
     * V2.9.31 CUS-2: 应用配色方案
     */
    public function applyColorScheme(): \think\Response
    {
        $data = $this->request->post();
        $memberId = (int) session('user_id');
        $themeSlug = $data['theme_slug'] ?? 'default';
        $schemeKey = $data['scheme_key'] ?? '';

        $service = new \app\common\service\template\TemplateColorSchemeService();
        $result = $service->apply($memberId, $themeSlug, $schemeKey);

        if ($result) {
            return $this->success('配色方案已应用');
        }
        return $this->error('应用失败');
    }
