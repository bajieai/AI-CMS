...
class SystemController extends AdminBaseController
{
    ...
    public function config()
    {
        ...
        if ($this->request->isGet()) {
            ...
            // V2.9.32: 配置项中文名fallback映射（remark为空时使用）
            $configNameFallback = [
                // system组
                'captcha_enabled'             => '是否启用验证码',
                'captcha_login'               => '登录时需要验证码',
                'captcha_register'            => '注册时需要验证码',
                'captcha_comment'             => '评论时需要验证码',
                'captcha_form'                => '表单提交时需要验证码',
                'captcha_fail_limit'          => '验证码失败重试次数限制',
                'admin_captcha_enabled'        => '后台登录验证码',
                'sse_max_connections_per_ip'   => 'SSE每IP最大连接数',
                'sse_max_connections_per_user' => 'SSE每用户最大连接数',
                'sse_connection_timeout'      => 'SSE连接超时时间(秒)',
                'sse_heartbeat_interval'      => 'SSE心跳间隔(秒)',
                'sse_message_ttl'             => 'SSE消息存活时间(秒)',
                'sse_offline_message_limit'   => 'SSE离线消息保留条数',
                'rss_enabled'                 => '启用RSS订阅',
                'rss_cache_ttl'               => 'RSS缓存时间(秒)',
                'app_version'                 => '系统版本号',
                'cdn_enabled'                 => '是否启用CDN',
                'cdn_domain'                  => 'CDN域名',
                'backup_keep_count'           => '自动备份保留最近N个',
                // email组
                'email_service_unified'       => '启用统一邮件服务',
                // oauth组
                'oauth_github_enabled'        => '启用GitHub登录',
                // ai组
                'image_provider'              => 'AI配图Provider',
                'image_api_key'               => 'AI配图API Key',
                'image_default_count'         => '默认生成配图数(1-5)',
                'image_default_style'         => '默认配图风格',
                'image_timeout'               => '配图API超时(秒)',
                'ai_stat_enabled'             => '是否启用AI生成统计',
                'ai_stat_retention_days'      => 'AI统计保留天数',
                'ai_image_default_provider'   => '默认AI配图Provider',
                'ai_image_fallback_provider'  => '备用AI配图Provider',
                'ai_image_flux_enabled'       => '是否启用FLUX配图',
                'ai_image_flux_api_key'       => 'FLUX API Key',
                'ai_image_flux_model'         => 'FLUX模型名称',
                'ai_image_dalle_enabled'      => '是否启用DALL-E配图',
                'ai_image_dalle_api_key'      => 'DALL-E API Key',
                'ai_image_dalle_model'        => 'DALL-E模型名称',
                'image_daily_limit'           => 'AI配图每日限额',
                'image_max_batch'             => 'AI配图批量最大数量',
                'writing_styles'              => 'AI写作风格配置',
                // content组
                'content_quality_check_enabled' => '启用AI内容质量检测',
                'sensitive_words_check_enabled'  => '启用敏感词过滤检测',
                'paid_content_enabled'          => '启用付费阅读功能',
                // pay组
                'pay_enabled'                 => '启用支付功能',
                'pay_wechat_enabled'          => '启用微信支付',
                'pay_alipay_enabled'          => '启用支付宝支付',
                // plugin组
                'license_verify_enabled'      => '启用许可证远程验证(插件商店)',
                // language组
                'auto_translate_enabled'      => '发布时自动AI翻译',
                'auto_translate_targets'      => '自动翻译目标语言(逗号分隔)',
                'translate_source_lang'       => '翻译源语言',
                // plugin组
                'market_url'                  => '插件市场API地址',
                'market_enabled'              => '启用插件市场',
                // pwa组
                'pwa_enabled'                 => '启用PWA离线支持',
                // seo组
                'sitemap_includes_cate'       => 'Sitemap包含分类页',
                'sitemap_includes_tag'        => 'Sitemap包含标签页',
                'schema_enabled'              => '启用结构化数据(JSON-LD)',
                'og_enabled'                  => '启用Open Graph标签',
                // member组
                'vip_free_read_mode'          => 'VIP免费阅读范围(0=不免费 1=全部免费)',
            ...
            unset($groups['points']);
            // 移除通知设置中的JSON字段（有专用可视化页面 /admin/notification/setting/index，避免暴露原始JSON）
            if (isset($groups['notification'])) {
                $groups['notification'] = array_filter($groups['notification'], function ($c) {
                    return !in_array($c['name'] ?? $c->name ?? '', ['notify_default_settings']);
                });
                if (empty($groups['notification'])) {
                    unset($groups['notification']);
                }
            }
    ...
    public function moduleControl()
    {
        ...
        if ($this->request->isGet()) {
            ...
            // V2.9.32: 统一图标格式，确保每条记录的icon只保留图标名（不含bi前缀）
