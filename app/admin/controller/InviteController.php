...
class InviteController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        // 邀请排行统计
        $rankQuery = Db::name('invite_relation')
        ...
        // 总体统计
        $stats = [
            'total_inviters' => Db::name('invite_relation')->field('COUNT(DISTINCT inviter_id) as c')->find()['c'] ?? 0,
            'total_invitees' => Db::name('invite_relation')->count(),
            'total_points'   => Db::name('invite_relation')->sum('reward_points') ?? 0,
        ];
