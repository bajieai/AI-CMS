...
class ContentController extends H5BaseController
{
    ...
    public function detail(): Json
    {
        ...
        // 推荐
        $recommend = Db::name('content')->where('status', 1)->where('cate_id', $content['cate_id'] ?? 0)->where('id', '<>', $id)->order('view_count', 'desc')->limit(5)->field('id,title,cover')->select()->toArray();
