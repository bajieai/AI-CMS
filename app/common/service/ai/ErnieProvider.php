...
/**
 * ERNIE(文心一言/百度千帆) Provider - V2.5新增
 * 百度千帆V2接口兼容OpenAI格式
 */
class ErnieProvider implements AiProviderInterface
{
    ...
    {
        $this->model = $model;
        $apiBase = $model->api_base ?: 'https://qianfan.baidubce.com/v2';
    ...
    public function write(string $prompt, array $options = []): string
    {
        ...
        $data = [
            'model' => $this->model->model_id ?? 'ernie-speed-128k',
            'messages' => $messages,
    ...
    {
        return ['provider' => 'ernie', 'model_id' => $this->model->model_id ?? 'ernie-speed-128k', 'capabilities' => explode(',', $this->model->capabilities ?? 'write,seo')];
    }
    ...
    protected function sendWithRetry(string $uri, array $data, int $retryCount = 0): array
    {
        try {
            ...
            throw new \Exception('连接文心一言服务失败');
            ...
            throw new \Exception('文心一言服务请求失败: ' . $e->getMessage());
        }
    }
}
