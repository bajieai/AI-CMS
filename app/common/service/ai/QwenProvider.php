...
class QwenProvider implements AiProviderInterface
{
    ...
    public function __construct(\app\common\model\AiModel $model)
    {
        ...
        $apiBase = $model->api_base ?: 'https://dashscope.aliyuncs.com/compatible-mode/v1';
    ...
    public function write(string $prompt, array $options = []): string
    {
        ...
        $data = [
            'model' => $this->model->model_id ?? 'qwen-plus',
            'messages' => $messages,
    ...
    public function getModelInfo(): array
    {
        return [
            'provider' => 'qwen',
            'model_id' => $this->model->model_id ?? 'qwen-plus',
            'capabilities' => $this->normalizeCapabilities(['write', 'seo', 'translate', 'summarize']),
        ];
    }

    /**
     * 带重试的HTTP请求（兼容OpenAI格式）
     */
    protected function sendWithRetry(string $uri, array $data, int $retryCount = 0): array
    {
        try {
            ...
            throw new \Exception('连接通义千问服务失败，请检查网络');
            ...
            if ($statusCode >= 400 && $statusCode < 500) {
                ...
                $message = $error['error']['message'] ?? $error['message'] ?? $e->getMessage();
                throw new \Exception("通义千问请求错误: {$message}");
            }

            throw new \Exception('通义千问请求失败: ' . $e->getMessage());
        }
    }
}
