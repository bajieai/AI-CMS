...
class TemplateBuyService
{
    ...
    public function createOrder(int $userId, int $templateId, string $tier = 'personal'): array
    {
        ...
        $orderId = Db::name('template_order')->insertGetId([
            'order_no' => $orderNo,
            'user_id' => $userId,
    ...
    {
        $exists = Db::name('template_cart')
            ->where('user_id', $userId)
            ->where('template_id', $templateId)
            ->find();
        if ($exists) return true;
        Db::name('template_cart')->insert([
            'user_id' => $userId,
            'template_id' => $templateId,
            'create_time' => date('Y-m-d H:i:s'),
        ]);
    ...
    {
        $items = Db::name('template_cart')->where('user_id', $userId)->select();
        $orders = [];
        foreach ($items as $item) {
            $orders[] = $this->createOrder($userId, (int) $item['template_id'], 'personal');
        }
        Db::name('template_cart')->where('user_id', $userId)->delete();
        Cache::delete('tpl_cart_' . $userId);
        return $orders;
    }
}
