...
class ContentImportExportService
{
        ...
        try {
            ...
            for ($i = 0; $i < $batches; $i++) {
                ...
                foreach ($rows as $row) {
                    ...
                    $rd = ['id'=>$row['id'],'title'=>$row['title'],'summary'=>$row['summary']??'','cate_id'=>$row['cate_id']??0,'status'=>$row['status'],'publish_time'=>$row['publish_time']??0];
    ...
    {
        $std=['title','summary','cate_id','status','publish_time','content','seo_title','seo_keywords','seo_description'];
    ...
    public function batchOperation(string $op, array $ids, array $params = []): array
    {
        try {
            ...
            switch($op){
                ...
                case 'moveCate': $c=Db::name('content')->whereIn('id',$ids)->update(['cate_id'=>(int)($params['cate_id']??0)]); break;
    ...
    public function getImportTemplate(int $modelId): array
    {
        ...
        $headers=['title','summary','cate_id','status','publish_time'];
