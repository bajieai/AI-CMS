...
class MeilisearchService
{
     ...
     * 获取索引名（动态拼接数据库前缀）
     */
    private static function getIndexName(): string
    {
        return \think\facade\Db::getConfig('prefix') . 'content';
    }
    ...
    public static function setupIndex(): bool
    {
        try {
            ...
            // 设置可搜索字段和过滤字段
            $client->put('indexes/' . self::getIndexName() . '/settings', [
                'json' => [
    ...
    public static function syncDocument(Content $content): bool
    {
        ...
        try {
            ...
            self::getClient()->put('indexes/' . self::getIndexName() . '/documents', [
                'json' => [$doc],
            ]);
    ...
    public static function deleteDocument(int $contentId): bool
    {
        ...
        try {
            self::getClient()->delete('indexes/' . self::getIndexName() . '/documents/' . $contentId);
            return true;
        } catch (\Exception $e) {
    ...
    public static function search(string $keyword, array $filters = [], int $page = 1, int $limit = 20): array
    {
        ...
        try {
            ...
            $response = self::getClient()->post('indexes/' . self::getIndexName() . '/search', [
                'json' => $body,
            ]);
