...
class FrontCsrfMiddleware
{
    ...
    protected array $exceptPaths = [
        ...
        'sendemailcode', // V2.9.59: 注册邮箱发码接口（同上）
        'oauth',
        'callback',
        'notify',
        'webhook',
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $prefix = env('DATABASE_PREFIX', Db::getConfig('prefix') ?: config('database.connections.mysql.prefix'));
        $cookieName = strtoupper(str_replace('_', '', $prefix)) . '_SID';
        if (empty($sessionToken) && !empty($_COOKIE[$cookieName])) {
            $sid = preg_replace('/[^a-f0-9]/', '', $_COOKIE[$cookieName]);
            // runtime_path() 在多应用模式返回 app/home/runtime/，实际session在项目根runtime
            $sessionPath = app()->getRootPath() . 'runtime/session/' . $prefix . '/sess_' . $sid;
