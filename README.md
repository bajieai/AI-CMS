...
- **后台 PJAX 内联脚本拼接陷阱根治** — V2.9.58 在 layout 加的全局块（clearCacheByType/openMediaSelect）被 `PjaxMiddleware::isLayoutGlobalScript` 的排除标记列表**遗漏**，PJAX 切页时被误当页面级 JS 与页面内联块拼接执行。**拼接陷阱**：全局块结尾的赋值表达式 `window.clearCacheByType = function(){...}`（无分号）紧跟页面块开头的 `(function(){...})()`，JS 把两者连成调用表达式 → 全局函数被误执行（SEO诊断页误弹"确定清除缓存吗"）→ 返回值 undefined 再被调 → `(intermediate value)(...) is not a function`。**双层修复**：①`isLayoutGlobalScript` 补 3 个全局块标记（layout 全局脚本不再进 PJAX 拼接）；②`pjax.js executeInlineScript` 内容末尾统一补分号（防御未来任何拼接陷阱）
...
- **双轨数据库迁移** — `install.sql` 种子更新（新装）+ 幂等升级补丁（存量在线升级自动执行，重复执行报 1060/1061 自动跳过）；该补丁现已统一并入 `install.sql`（member 表建表语句直接含 `mobile` 字段与 `uk_mobile` 唯一索引），database 目录仅保留 install.sql
...
- **列表页 TypeError 修复** — `date('Y-m-d', $vo.create_time)` 当数据库字段是历史字符串（外部导入/老版本迁移）时，PHP 8 严格类型检查报 "Argument #2 must be of type ?int, string given"。**根因**：`Content` 模型的 `protected $type` 数组只控制 `views/status` 等业务字段类型转换，**没有** `create_time`/`update_time`/`publish_time`，`$autoWriteTimestamp='int'` 只控制**写入**而不控制**读取**。**根解**：`Content::$type` 补三个时间字段为 `integer`（模型访问器层强制转换，所有路由生效），并对模板里 `date()` 调用加 `(int)` 双保险
- **移动端 index.html 隐藏 bug 清除** — V2.9.31 创建的 `default/mobile/index.html` 和 `corporate/mobile/index.html` 第 1 行有**残留的 `<?php` 包装**，导致 ThinkPHP 模板标签 `{extend}` 解析时 PHP 引擎处于打开状态并报 "unexpected token <"（这个 bug 一直没触发是因为首页走的是 layout 兜底路径，但 V2.9.49 起的 4 套主题对应模板会显式 include，触发概率上升）
- **模板编译验证脚本** — `.codebuddy/test_compile_249.php` 用 ThinkPHP 模板编译器（反射调 `parse` 方法）+ PHP `php -l` lint 验证 21 个真实可渲染模板全部通过编译，剩余 `template_store/` 下的 5 个文件是模板编辑器源码展示文件（含 `<?php` 是有意的），不影响运行
